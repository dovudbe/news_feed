...
class EditUserView(LoginRequiredMixin, View):
    def get(self, request):
        user_form = UserEditForm(instance= request.user)
        frofilw_form = ProfileEditForm(instance= request.user.profile)

        return render(request, 'account/profile_edit.html', {'user_form':user_form, 'profile_form':profile_form})


    def Post(self, request):
        user_form = UserEditForm(instance= requset.user, date= request.POST)
        profile_form = ProfileEditForm(instance= request.user.profile, date= request.POST, files = requset.Files)
